...
#'
#' An object returned by the \code{\link{tvure}} function: this is a list
...
#' \item{\code{regr1} : \verb{ }}{List returned by the \code{\link{DesignFormula}} function for the linear predictor associated to \code{formula1}.}
#' \item{\code{regr2} : \verb{ }}{List returned by the \code{\link{DesignFormula}} function for the linear predictor associated to \code{formula2}.}
...
#'   \item{\code{res} : \verb{ }}{Standardized residual \eqn{(event_{ij}-\mu_{ij})/\sqrt{\mu_{ij}} for unit \eqn{i} at time \eqn{t_{ij}} where \eqn{\mu_{j}=h_p(t_{ij}|z(t_{ij}),x(t_{ij}))} and \eqn{event_{ij}} is the event indicator.}
#'   \item{\code{phi} : \verb{ }}{Vector of length \eqn{K_0} containing the estimated B-splines coefficients in \eqn{\log f_0(t)}.}
#'   \item{\code{beta} : \verb{ }}{Matrix containing the point estimates, standard errors, credible intervals, Z-scores and P-values of the regression and spline parameters in the long-term (cure) survival (or quantum) submodel.}
#'   \item{\code{gamma} : \verb{ }}{Matrix containing the point estimates, standard errors, credible intervals, Z-scores and P-values of the regression and spline parameters in the short-term (cure) survival (or timing) submodel.}
